$(function(){
	$('#txtP_Moisture').blur(function(){
		...
		if($.trim($('#txtP_A').val())!= '' && $.trim($('#txtP_B').val())!='' && $.trim($('#txtP_E').val())!=''){
			p_a = parseFloat($.trim($('#txtP_A').val()));
			p_b = parseFloat($.trim($('#txtP_B').val()));
			p_e = parseFloat($.trim($('#txtP_E').val()));
			p_result = (p_e/(p_a*p_b))
			$('#txtP_F').val(p_result.toFixed(4));
		}
		if($.trim($('#txtO_A').val())!= '' && $.trim($('#txtO_B').val())!='' && $.trim($('#txtO_E').val())!=''){
			o_a = parseFloat($.trim($('#txtO_A').val()));
			o_b = parseFloat($.trim($('#txtO_B').val()));
			o_e = parseFloat($.trim($('#txtO_E').val()));
			o_result = (o_e/(o_a*o_b))
			$('#txtO_F').val(o_result.toFixed(4));
		}
		if($.trim($('#txtP_F').val())!=''){
			p_f = parseFloat($.trim($('#txtP_F').val()));
			p_result = p_f * 4046;
			$('#txtP_G').val(p_result.toFixed(4));
		}
		if($.trim($('#txtO_F').val())!=''){
			o_f = parseFloat($.trim($('#txtO_F').val()));
			o_result = o_f * 4046;
			$('#txtO_G').val(o_result.toFixed(4));	
		}
		if($.trim($('#txtP_G').val()) != '' && $.trim($('#txtO_G').val()) != ''){
			p = parseFloat($.trim($('#txtP_G').val()));
			o = parseFloat($.trim($('#txtO_G').val()));
			diff_ = p - o; 
			$('#total_difference').html(diff_.toFixed(4));
		}
	});
